...
class Strategy(CtaTemplate):
    ...
    # 变量列表
    varList = ['transactionPrice','entrySig_List','exitSig_List']  
    ...
    def stoploss2(self,bar,ATRPeriod):
        arrayPrepared1,amSignal=self.arrayPrepared(ATRPeriod)
        if arrayPrepared1:
            algorithm=maSignal()
            sVolty=algorithm.sVolty(amSignal,self.paraDict)
        if self.posDict[self.symbol+'_LONG']>0:
            if sVolty==-1:
                self.cancelAll()
                self.sell(self.symbol, bar.close*0.99, self.posDict[self.symbol+'_LONG'])
    ...
    def strategy(self, bar):
        ...
        ATRPeriod=self.timeframeMap["ATRPeriod"]
        ...
        '''
        self.entrySig_List.append(entrySig)
        self.exitSig_List.append(exitSig)
